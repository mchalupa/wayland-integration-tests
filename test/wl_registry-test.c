		       ...
		       uint32_t id, const char *interface, uint32_t version)
{
	...
	if (strcmp(interface, "wl_seat") == 0) {
		...
		wit_client_add_listener(cl, "wl_seat", &seat_listener);
...
TEST(global_remove_tst)
{
	struct wit_display *d
		= wit_display_create_and_run(NULL, global_remove_main);

	wl_seat_send_name(d->resources.seat, "Cool name");
	wl_seat_send_name(d->resources.seat, "Cool name2");

	dbg("Deleting global\n");
	wl_global_destroy(d->globals.wl_seat);
	wit_display_barrier(d);

	wl_seat_send_name(d->resources.seat, "destroy");
	wl_seat_send_name(d->resources.seat, "destroy1");
	wl_seat_send_name(d->resources.seat, "destroy2");
	wit_display_barrier(d);
...
static void
registry_handle_global2(void *data, struct wl_registry *registry,
			uint32_t id, const char *interface, uint32_t version)
{
...
const struct wl_registry_listener registry_listener2 = {
	registry_handle_global2,
	NULL
};
...
TEST(global_tst)
{
	...
	wit_display_create_client(d, global_main);
	wit_display_run(d);
