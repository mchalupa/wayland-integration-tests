		       ...
		       uint32_t id, const char *interface, uint32_t version)
{
	...
	if (strcmp(interface, "wl_shm") == 0) {
		...
		wit_client_add_listener(cl, "wl_shm", (void *) &format_listener);
...
format_emit_main(int s)
{
	...
	wit_client_add_listener(&c, "wl_registry", (void *) &registry_listener);
...
{
	struct wit_config conf = {CONF_SHM,CONF_SHM,0};
	struct wit_display *d =
		wit_display_create_and_run(&conf, format_emit_main);
